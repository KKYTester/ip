...
`scripts/generate-split-view-diff.py` is the bundled standard-library-only generator, based on [se-edu/skill-present-changes-visually](https://github.com/se-edu/skill-present-changes-visually). No third-party Python package is required. Keep the page self-contained except for optional syntax-highlighting resources loaded by the page.
